...
and HCL/Terraform's `*.tftest.hcl`)
...
By default, rinkaku skips generated files three ways:
...
- **Terraform lockfile path:** `.terraform.lock.hcl` is treated as
  generated by path (no content marker needed); `--include-generated`
  overrides it like the other checks.
...
Pass `--include-generated` to disable all three detections.
...
- **HCL support is Terraform-specific.** HCL support covers Terraform
  syntax (`.tf`, `.tofu`, `.tftest.hcl`); plain `.hcl` dialects (Packer,
  Nomad) and JSON-syntax `.tf.json` files are skipped as unsupported.
