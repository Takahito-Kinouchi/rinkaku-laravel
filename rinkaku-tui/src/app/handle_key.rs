//! Key-event dispatch: [`App::handle_key`], split out of `app/mod.rs`
//! (ADR 0028) since it is the single largest responsibility on `App` —
//! routing every [`InputKey`] against `Screen`/`Focus` state. Struct/enum
//! definitions and state accessors stay in `app/mod.rs`/`app/state.rs`;
//! the three sibling overlay/popup dispatch methods this function defers
//! to (`handle_review_key`, `handle_key_with_popup_open`,
//! `handle_scroll_key`) live in `app/review_key.rs`, `app/jump.rs`, and
//! `app/scroll_key.rs` respectively.
