...
fn right_focus_bindings(locale: Locale) -> Vec<KeyBinding> {
    ...
    vec![
        ...
        // ADR 0088's amendments: with the pane focused a file row reads its
        // whole diff in one motion rather than stopping where its first
        // symbol begins — there is no tree walk here to read the rest — so
        // the two groups no longer share one description.
        KeyBinding {
            keys: "ctrl-f / ctrl-b",
